                    ...
                    <table id="table" class="table" cellspacing="0" width="100%">
                        <thead>
                            <tr>
                                ...
                                <th width="300">Jenis Dokumen</th>
                                <th width="300">Nomor Dokumen</th>
                                <!-- <th>File</th> -->
                                <th>Tanggal</th>
                                <th>User Upload</th>
                ...
                <div class="row">
                    ...
                    <ul class="list-group">
                       <li class="list-group-item">
                            ...
                            <div class="form-group">
                                <label id="title"><b>Replace Dokumen</b></label>
...
<script type="text/javascript">
  ...
  if ($(event.target).is("button")) {
    ...
    function view_dokumen(id) {
        ...
        $.ajax({
            url: "<?= site_url('user/dokumen_view/') ?>" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data) {
                ...
                $('.modal-title').text('View Arsip Dokumen'); // Set title to Bootstrap modal title
    ...
    function save() {
        var url;
        if (save_method == 'add') {
            url = "<?= site_url('user/dokumen_add') ?>";
        } else {
            url = "<?= site_url('user/dokumen_update') ?>";
        }
    ...
    function delete_dokumen(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: "<?= site_url('user/dokumen_delete') ?>/" + id,
                    type: "POST",
                    dataType: "JSON",
                    success: function(data) {
                        if (data.response == 'error' || data.error) {
    ...
    function get_list_pihak(id){
		...
		$.ajax({
			url : "<?=site_url('user/get_pihak_list/')?>" + id,
			type: "GET",
			dataType: "json",
			async: false,
			success: function(data)
