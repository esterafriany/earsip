...
class SuratInternal extends CI_Model {
    ...
    function get_surat_destinasi($table, $data=array()){
        $this->db->select('
            id_surat_internal,
        ...
        ');
        $this->db->from('surat_internal');
        $this->db->join('user pengirim','pengirim.id_user = surat_internal.asal_surat','left');
        $this->db->join('pegawai pegawai_pengirim','pegawai_pengirim.id_pegawai = pengirim.id_pegawai','left');
        $this->db->join('jabatan pengirim_jabatan','pengirim_jabatan.id_jabatan = pegawai_pengirim.id_jabatan','left');
        $this->db->join('unit_kerja pengirim_unit','pengirim_unit.id_unit = pengirim_jabatan.id_unit','left');
        $this->db->join('jenis_surat','jenis_surat.id_jenis = surat_internal.id_jenis','left');
        $this->db->join('prioritas_surat','prioritas_surat.id_prioritas = surat_internal.id_prioritas','left');
        $this->db->join('sifat_surat','sifat_surat.id_sifat = surat_internal.id_sifat','left');
        $this->db->join('media_surat','media_surat.id_media = surat_internal.id_media','left');
        $this->db->where($data);
    ...
    function join_disposisi_pegawai($query){
        $this->db->select("*");
        $this->db->join('perintah_disposisi','perintah_disposisi.id_perintah = disposisi_internal.id_perintah');
        $this->db->join('pegawai','pegawai.id_pegawai = disposisi_internal.tujuan_disposisi');
        $this->db->where('tujuan_disposisi',$query);
        return $this->db->get('disposisi_internal');
    }
